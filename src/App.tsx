...
📱 Verified using HalalVerify Mobile Sandbox`;
  ...
  const renderAppViewport = () => {
    return (
      <div 
        ...
        onDrop={handleDrop}
        className={`relative w-full h-full bg-[#f8fafc] dark:bg-stone-950 overflow-hidden flex flex-col text-stone-850 dark:text-stone-100 select-text ${
          isNative ? "" : "rounded-[34px]"
        }`}
      >
        
        {/* Phone Status Bar (Simulated only for web preview browser, hide natively on target) */}
        {!isNative && (
                    ...
                    result && (
                      <div className="space-y-3">
                        ...
                        <div className="bg-white border rounded-xl overflow-hidden dark:bg-stone-900 dark:border-stone-800">
                          <div className="px-3 py-1.5 bg-slate-50 border-b text-[8px] tracking-wider uppercase font-black text-slate-400 dark:bg-slate-850">Chemical / Additive Breakdown</div>
                    ...
                    <div className="p-3.5 space-y-4 flex-1 flex flex-col">
                      ...
                      {!developerKeyAvailable && !useCustomKey && (
                        <div className="p-2 bg-amber-50 rounded-lg border border-amber-100/60 text-left shrink-0">
                          <span className="text-[8px] font-black text-amber-805 uppercase block">Sandbox Offline Mode</span>
                      ...
                      <div className="bg-white border rounded-xl p-2.5 text-left space-y-1.5 dark:bg-stone-900 border-slate-150">
                        <span className="text-[8.5px] text-slate-405 font-black uppercase">Active Snap Batch</span>
                        {uploadedImages.length === 0 ? (
                          <span className="text-[9.5px] italic text-slate-404 block py-1">No pictures snapped yet.</span>
                        ) : (
                          <div className="flex items-center gap-2 overflow-x-auto pb-1 select-none">
                            {uploadedImages.map(img => (
                              <div 
                        ...
                        {filteredBrandsList.map(comp => (
                          <div key={comp.id} className="p-2 bg-white border border-slate-200 rounded-lg dark:bg-stone-905">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-[7px] font-bold uppercase text-slate-404">{comp.category}</span>
                            </div>
                            <div className="mt-1.5 border-l border-dashed border-emerald-400 pl-2 text-[9.5px]">
                      ...
                      <div className="space-y-2 pr-0.5 pb-4">
                        {[
                          ...
                          { 
                            ...
                            badgeClass: "bg-sky-100/90 text-sky-850 border-sky-250 dark:bg-sky-950/50 dark:text-sky-400 dark:border-sky-900",
                  ...
                  {simulatorTab === "settings" && (
                    <div className="p-4 flex-1 flex flex-col space-y-3.5 text-left">
                      ...
                      <div className="space-y-3">
                        <label className="flex items-start gap-1.5 cursor-pointer">
                          ...
                          <div className="text-[10.5px]">
                            ...
                            <p className="text-[8.5px] text-slate-405 font-medium">Provide API credential to execute calls.</p>
                          </div>
                        </label>
                        ...
                        <div className="pt-2 border-t border-slate-100 dark:border-stone-800">
                          <label className="flex items-start gap-1.5 cursor-pointer">
                            ...
                            <div className="text-[10.5px]">
                              ...
                              <p className="text-[8.5px] text-slate-405 font-medium">Validate brand declarations against web indices.</p>
                            </div>
                          </label>
                        </div>
                      </div>
                      ...
                      <button 
                        ...
                        className="w-full py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg text-[10px] cursor-pointer"
                      >
                        Save settings
                      </button>
                    </div>
  ...
  if (isNative) {
    return (
      <div id="halal-pro-app" className="w-full h-screen bg-[#f8fafc] dark:bg-stone-950 font-sans text-stone-850 select-none dark:text-stone-105 transition-colors duration-300 flex flex-col">
          ...
          <div className="px-5 py-3 bg-slate-50 border-b flex justify-between items-center dark:bg-stone-850 dark:border-stone-800">
            <div className="flex items-center gap-2 text-left">
              ...
              <div>
                ...
                <span className="text-[9px] text-stone-405">Ready-compilable React Native TSX blocks for Expo iOS/Android</span>
              </div>
            </div>

            <button
              onClick={() => handleCopyCode(activeCodeFileIndex, activeCodeFile.content)}
              className="flex items-center gap-1 px-3 py-1 bg-white border hover:bg-slate-50 border-slate-200 text-stone-705 text-[10px] font-bold rounded-lg shadow-sm transition dark:bg-stone-800 dark:border-stone-700 dark:text-stone-105 cursor-pointer"
            >
            ...
            {REACT_NATIVE_FILES.map((file, idx) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-2 text-[10.5px] font-semibold flex items-center gap-1.5 whitespace-nowrap transition cursor-pointer border-r border-slate-205 dark:border-stone-850 ${
                    isActive 
